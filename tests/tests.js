...
QUnit.test( "method getNextTagFromHtml()", function( assert ) {
	...
	assert.ok( !tagMatchTest.getNextTagFromHtml( 'klsjdklfjsf' ), "text string" );
	assert.ok( !tagMatchTest.getNextTagFromHtml( '' ), "empty string" );
	assert.ok( !tagMatchTest.getNextTagFromHtml( '    /header>' ), "almost closing tag \"/header\"" );
});
